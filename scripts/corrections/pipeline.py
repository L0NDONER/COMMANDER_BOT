...
imports those five and runs them in the listed order. CLI:
    ...
    python3 scripts/corrections/pipeline.py  scripts/gps_traces/walkbubble_demo_2026-05-29.json
    python3 scripts/corrections/pipeline.py  <trace>  --scorer agreement
    python3 scripts/corrections/pipeline.py  <trace>  --scorer mode  --k 2  --L 10
"""
import argparse
import json
import sys
...
# Layer 1 — motion
from walker_arrow_test import classify_legs  # noqa: E402
...
def run(trace_path: str,
        ...
        n_perm: int = 2000,
        seed: int = 0) -> dict:
    """Single-trace end-to-end run. Returns a dict with the per-layer
    artefacts: `{seq, weight, scorer, falsification}`."""
    # 1. motion
    seq = classify_legs(trace_path)
...
def main() -> None:
    ...
    args = ap.parse_args()
    out = run(args.trace, scorer_name=args.scorer, k=args.k, L=args.L,
